...
def build_value_function(input_shape):
    input_layer = Input(shape=input_shape)
    x = Dense(64, activation="relu")(input_layer)
...
class PGAgent:
    def __init__(self, env_name):
        ...
        self.value_function = build_value_function(input_shape=self.state_size)
        self.actor_optim = Adam(self.actor_lr)
        self.value_optim = Adam(self.value_lr)
    ...
    def replay(self):
        ...
        with tf.GradientTape() as tape:
            baseline_values = self.value_function(states)
            loss = tf.reduce_mean(tf.square(discounted_r - baseline_values))
        grads = tape.gradient(loss, self.value_function.trainable_weights)
        self.value_optim.apply_gradients(zip(grads, self.value_function.trainable_weights))
        ...
        with tf.GradientTape() as tape:
            ...
            loss = tf.reduce_mean(-tf.math.log(selected_action_prob) * (discounted_r - baseline_values))
    ...
    def run(self):
        for e in range(self.max_episodes):
            ...
            if e >= 3 and np.mean(self.episode_rewards[:e+1][-3:]) >= 500.0:
                break
